...
build do
  ...
  command "mkdir -p #{install_path}/embedded/cookbooks"
  command "#{install_path}/embedded/bin/berks install --berksfile=./Berksfile --path=#{install_path}/embedded/cookbooks",
          :env => { "RUBYOPT"         => nil,
                    "BUNDLE_BIN_PATH" => nil,
                    "BUNDLE_GEMFILE"  => nil,
                    "GEM_PATH"        => nil,
                    "GEM_HOME"        => nil }
  block do
    File.open("#{install_path}/embedded/cookbooks/dna.json", "w") do |f|
      f.puts "{\"run_list\": [ \"recipe[#{project_name}]\" ]}"
    end
    File.open("#{install_path}/embedded/cookbooks/show-config.json", "w") do |f|
      f.puts "{\"run_list\": [ \"recipe[#{project_name}::show_config]\" ]}"
    end
    File.open("#{install_path}/embedded/cookbooks/post_upgrade_cleanup.json", "w") do |f|
      f.puts "{\"run_list\": [ \"recipe[#{project_name}::post_11_upgrade_cleanup]\" ]}"
    end
    File.open("#{install_path}/embedded/cookbooks/solo.rb", "w") do |f|
